...
def colorize(data, color):
    ...
    if color not in colors:
        raise KeyError(f"No such color: `{color}`")
...
def set_logger_level(level='info'):
    if level.lower() == 'info':
        logging.getLogger('kr_logger_root').setLevel("INFO")
        logger.info("Logging level: INFO")
    elif level.lower() == 'debug':
        logging.getLogger('kr_logger_root').setLevel("DEBUG")
...
logger = logging.getLogger('kr_logger_root')
logger.setLevel(logging.DEBUG)
